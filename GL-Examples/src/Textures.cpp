...
GL_EXAMPLE_DECLARE("Textures")

GL_EXAMPLE_IMPLEMENTATION {
	constexpr int quad_count = 32;
	...
	// UV coordinates in the generic vertex shader are in layout 2
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(float) * 5, (void*)(sizeof(float) * 3));
	glEnableVertexAttribArray(2);

	Shader shader("shaders/vs_generic.glsl", "shaders/fs_generic.glsl");
	shader.Use();
